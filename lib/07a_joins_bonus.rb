...
def alison_artist
  ...
  execute(<<-SQL)
  SELECT albums.artist FROM albums
...
def blur_songs
  ...
  execute(<<-SQL)
  SELECT tracks.song FROM tracks
...
def heart_tracks
  ...
  execute(<<-SQL)
  SELECT DISTINCT albums.title, COUNT(*) FROM albums
...
def song_title_counts
  ...
  execute(<<-SQL)
  SELECT tracks.song, COUNT(DISTINCT albums.title) FROM tracks
  JOIN albums ON tracks.album = albums.asin
...
def best_value
  ...
  # of tracks.
  execute(<<-SQL)
  SELECT albums.title, albums.price, COUNT(*) FROM albums
  JOIN tracks ON albums.asin = tracks.album
...
def top_track_counts
  ...
  execute(<<-SQL)
  SELECT albums.title, COUNT(tracks.song) FROM albums
  JOIN tracks ON albums.asin = tracks.album
  GROUP BY albums.title
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT albums.artist, COUNT(DISTINCT albums.title) FROM albums
  JOIN styles ON styles.album = albums.asin
  WHERE styles.style LIKE '%Rock%'
  GROUP BY albums.artist
  ORDER BY COUNT(DISTINCT albums.title) DESC LIMIT 1
  SQL
end
