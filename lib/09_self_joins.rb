...
def cl_to_lr
  ...
  WHERE
    a.stop_id = 53 AND
    b.stop_id = (SELECT stops.id FROM stops WHERE stops.name='London Road')
  SQL
end
...
FROM
(SELECT a.company, a.num, b.stop_id
 FROM routes a JOIN routes b ON (a.company=b.company AND a.num=b.num)
 WHERE a.stop_id=(SELECT id FROM stops WHERE name= 'Craiglockhart')
)S
  JOIN
(SELECT a.company, a.num, b.stop_id
 FROM routes a JOIN routes b ON (a.company=b.company AND a.num=b.num)
 WHERE a.stop_id=(SELECT id FROM stops WHERE name= 'Sighthill')
)E
ON (S.stop_id = E.stop_id)
JOIN stops ON(stops.id = S.stop_id)
  SQL
end
